...
import { Loader2, User, Lock, AlertCircle, Eye, EyeOff, Info } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleIdentifierChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Just keep the digits, no formatting
      ...
      setValue('identifier', digitsOnly, { shouldValidate: false });
    }
  };
  ...
  return (
    ...
    <Card className="bg-white/75 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-3xl border border-white/40 ring-1 ring-black/5">
      ...
      <CardContent>
        ...
        <div className="mt-6 rounded-xl bg-slate-50 p-4 text-xs border border-slate-100 shadow-sm">
          <p className="mb-2 font-semibold text-slate-700 flex items-center gap-1.5">
            <Info className="h-3.5 w-3.5 text-cyan-600" />
            Panduan Format Login
          </p>
          <div className="space-y-2 text-slate-600">
            <div>
              <p className="font-semibold text-slate-800">Admin / Keuangan / Dosen:</p>
              <p className="ml-2 text-slate-500">• Gunakan <span className="font-mono bg-slate-200/60 px-1 rounded">Username Resmi</span> atau <span className="font-mono bg-slate-200/60 px-1 rounded">NIDN (10 digit)</span> / <span className="font-mono bg-slate-200/60 px-1 rounded">NUPTK (16 digit)</span> tanpa tanda baca.</p>
            </div>
            
            <div>
              <p className="font-semibold text-slate-800">Mahasiswa:</p>
              <p className="ml-2 text-slate-500">• Masukkan <span className="font-mono bg-slate-200/60 px-1 rounded">NIM</span> langsung tanpa titik (Contoh: ketik <span className="font-mono font-semibold text-slate-700">2401001</span> akan otomatis diformat menjadi <span className="font-mono font-semibold text-slate-700">24.01.001</span>).</p>
            </div>
            
            <div className="mt-3 italic border-t border-slate-200/60 pt-2 text-slate-500">
              * Jika baru pertama kali login, gunakan password default yang diberikan oleh pihak Akademik/BAAK.
            </div>
          </div>
        </div>
