...
export default function KelasMKListPage() {
  ...
  // DELETE DIALOG STATE
  // ============================================
  const [deleteDialog, setDeleteDialog] = useState<{ open: boolean; id: number | null; namaMK: string }>({
    open: false,
    id: null,
    namaMK: '',
  });
  ...
  const handleDeleteClick = useCallback((id: number, namaMK: string) => {
    setDeleteDialog({ open: true, id, namaMK });
  }, []);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await kelasMKAPI.delete(deleteDialog.id);

      if (response.success) {
        toast.success('Kelas berhasil dihapus');
        setDeleteDialog({ open: false, id: null, namaMK: '' });
        fetchKelasMK();
      } else {
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleDeleteClick(kelas.id, kelas.mataKuliah?.namaMK || 'N/A');
                            }}
                          >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold">&quot;{deleteDialog.namaMK}&quot;</span>?
              <br />
