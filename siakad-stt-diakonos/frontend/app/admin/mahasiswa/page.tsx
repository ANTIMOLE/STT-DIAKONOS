...
export default function MahasiswaPage() {
  ...
  const [deactivateDialog, setDeactivateDialog] = useState<{ open: boolean; id: number | null; nama: string }>({
    open: false,
    id: null,
    nama: '',
  });
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await mahasiswaAPI.delete(deleteDialog.id);
  ...
  const handleDeactivateClick = useCallback((id: number, nama: string) => {
    setDeactivateDialog({ open: true, id, nama });
  }, []);
  ...
  const confirmDeactivate = useCallback(async () => {
    ...
    try {
      ...
      const response = await mahasiswaAPI.update(deactivateDialog.id, { status: 'NON_AKTIF' });

      if (response.success) {
        toast.success('Mahasiswa berhasil dinonaktifkan');
        setFilters(prev => ({ ...prev, page: prev.page }));
        setDeactivateDialog({ open: false, id: null, nama: '' });
      } else {
  ...
  return (
    <div className="space-y-6">
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-right">
                          <MahasiswaActions
                            ...
                            onDeactivate={() => handleDeactivateClick(item.id, item.namaLengkap)}
                            onDelete={() => handleDeleteClick(item.id, item.namaLengkap)}
                          />
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold">{deleteDialog.nama}</span>?
              <br />
      ...
      <AlertDialog open={deactivateDialog.open} onOpenChange={(open) => setDeactivateDialog({ ...deactivateDialog, open })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Nonaktifkan Mahasiswa?</AlertDialogTitle>
            <AlertDialogDescription>
              Apakah Anda yakin ingin menonaktifkan mahasiswa{' '}
              <span className="font-semibold">{deactivateDialog.nama}</span>?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
